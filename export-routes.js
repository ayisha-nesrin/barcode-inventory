...
router.get('/products.xlsx', async (req, res) => {
  ...
  db.get('products').value().forEach((p) => sheet.addRow(p));
...
router.get('/scans.xlsx', async (req, res) => {
  const workbook = new ExcelJS.Workbook();
  ...
  sheet.getRow(1).font = { bold: true };
  db.get('scans').value().forEach((s) => sheet.addRow(s));
...
router.get('/products.pdf', (req, res) => {
